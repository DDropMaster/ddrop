...
namespace DDrop.BL.ImageProcessing.CSharp
{
    ...
    {
        public Point[] GetDiameters(byte[] image, Rectangle cropRegion, int ksize = 9, int treshold1 = 50, int treshold2 = 100,
            int size1 = 100, int size2 = 250)
        {
            ...
            if (cropRegion != Rectangle.Empty)
            {
                imageInput.ROI = cropRegion;
            }
            ...
            if (cropRegion != Rectangle.Empty)
            {
                ...
                {
                    arr[0][i].X += cropRegion.X;
                    arr[0][i].Y += cropRegion.Y;
                }
            }
